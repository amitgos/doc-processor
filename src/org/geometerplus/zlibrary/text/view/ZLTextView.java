...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	private  ZLTextPage curPage = myCurrentPage;
...
//		Log.i("Start Cursor Info", String.valueOf(page.StartCursor.getElementIndex())+ " " + 
//				String.valueOf(page.StartCursor.getParagraphIndex()) + " " + 
//				String.valueOf(page.StartCursor.getCharIndex() ));
//		Log.i("End Cursor Info", String.valueOf(page.EndCursor.getElementIndex())+ " " + 
//				String.valueOf(page.EndCursor.getParagraphIndex()) + " " + 
//				String.valueOf(page.EndCursor.getCharIndex() ));
		...
		for (ZLTextLineInfo info : lineInfos) {
			...
			curLineInfo = info;
			OFFSET = info.Height + info.Descent + info.VSpaceAfter;
			
			if(ZLAndroidWidget.CROP &&  ZLAndroidWidget.TOUCH){
				
				if((ZLAndroidWidget.y1 < y+OFFSET && ZLAndroidWidget.y1 >= y) && !y1){
					Log.i("testing Y1 value",String.valueOf(ZLAndroidWidget.y1));
					Log.i("testing Y value",String.valueOf(y));
					startLineInfo = curLineInfo;
					y1=true;
				}
				
				if((ZLAndroidWidget.y2 < y+OFFSET && ZLAndroidWidget.y2 >= y) && !y2){
					Log.i("testing Y2 value",String.valueOf(ZLAndroidWidget.y2));
					Log.i("testing Y value",String.valueOf(y));
					endLineInfo = curLineInfo;
					y2=true;
				}
				
			if(y1 && y2){
				ZLAndroidWidget.TOUCH = false;
				y1=false;
				y2=false;
				getText();
			}
			...
			if( (startLineInfo != null )&& (curLineInfo == startLineInfo )){
				mY1 = y;
				DRAW1 = true;
			}else{
//				DRAW1 = false ;
	...
	public static ZLTextLineInfo startLineInfo,endLineInfo,curLineInfo;
	private static boolean y1= false, y2=false;
	public static String TEXT;
	private int OFFSET ;
	public static  int mY1,mY2;
	public static boolean DRAW1 = false , DRAW2 = false;
		
		public void getText(){
			
			Log.i("testing Text","getText");
			ZLTextWordCursor cursor = getStartCursor();
			cursor.moveToParagraph(startLineInfo.ParagraphCursor.Index);
			cursor.moveTo(startLineInfo.RealStartElementIndex, startLineInfo.RealStartCharIndex);
			Log.i("startLineInfo start Element Index",String.valueOf(startLineInfo.StartElementIndex));
			Log.i("endinfo start Element Index", String.valueOf(endLineInfo.StartElementIndex));
			String data = createBookmarkText(cursor,endLineInfo.StartElementIndex);
			TEXT = data ;
			Log.i("testing TEXt", String.valueOf(data.length()) + data);
			
		}
		
	private String createBookmarkText(ZLTextWordCursor cursor,int endElement) {
		...
		Log.i("start Info Element",String.valueOf(cursor.getElementIndex()));
		Log.i(" end Element",String.valueOf(endElement));
		
	mainLoop:
		while (( cursor.getElementIndex()!= endElement)) {
...
//	// ********************************************* END OF MODIFIED CODE  *****************************************************************
	...
	public final synchronized int getScrollbarThumbLength(int viewPage) {
		...
		int start = (scrollbarType() == SCROLLBAR_SHOW_AS_PROGRESS) ? 0
				: sizeOfTextBeforeCursor(page.StartCursor);
		if (start == -1) {
			start = 0;
		}
	...
	private int sizeOfTextBeforeCursor(ZLTextWordCursor wordCursor) {
		...
		if (paragraphLength > 0) {
			sizeOfText += (myModel.getTextLength(paragraphIndex) - sizeOfText)
					* cursor.getElementIndex() / paragraphLength;
		}
		return sizeOfText;
	}
	...
	private void drawTextLine(ZLTextPage page, ZLTextLineInfo info, int from,
		...
		if ((page == myCurrentPage) && !mySelectionModel.isEmpty()
				&& (from != to)) {
			final int paragraphIndex = paragraph.Index;
			final ZLTextSelectionModel.Range range = mySelectionModel
					.getRange();
			...
			if (paragraphIndex > lBound.ParagraphIndex) {
				...
				if (info.StartElementIndex > boundElementIndex) {
					left = getLeftMargin();
						...
						|| ((info.EndElementIndex == boundElementIndex) && (info.EndCharIndex >= lBound.CharIndex))) {
					final ZLTextElementArea elementArea = page.findLast(from,
							to, lBound);
					left = elementArea.XStart;
					if (elementArea.Element instanceof ZLTextWord) {
						left += getAreaLength(paragraph, elementArea,
								lBound.CharIndex);
					}
				}
			}
			...
			if (paragraphIndex < rBound.ParagraphIndex) {
				...
				if ((info.EndElementIndex < boundElementIndex)
						|| ((info.EndElementIndex == boundElementIndex) && (info.EndCharIndex < rBound.CharIndex))) {
					right = getRightLine();
					bottom += info.VSpaceAfter;
				} else if ((info.StartElementIndex < boundElementIndex)
						|| ((info.StartElementIndex == boundElementIndex) && (info.StartCharIndex <= rBound.CharIndex))) {
					final ZLTextElementArea elementArea = page.findLast(from,
							to, rBound);
					if (elementArea.Element instanceof ZLTextWord) {
						right = elementArea.XStart
								+ getAreaLength(paragraph, elementArea,
										rBound.CharIndex) - 1;
					} else {
		...
		for (int wordIndex = info.RealStartElementIndex; (wordIndex != endElementIndex)
			...
			// if ((element instanceof ZLTextWord) || (element instanceof
			// ZLTextImageElement)) {
			if (element == area.Element) {
				...
				if (element instanceof ZLTextWord) {
					drawWord(areaX, areaY, (ZLTextWord) element, charIndex, -1,
							false);
		...
		if (index != to) {
			...
			drawWord(area.XStart, area.YEnd - context.getDescent()
					- getTextStyle().getVerticalShift(), word, 0, len,
	...
	private void buildInfos(ZLTextPage page, ZLTextWordCursor start,
		...
		do {
			...
			while (info.EndElementIndex != endIndex) {
				info = processTextLine(paragraphCursor, info.EndElementIndex,
						info.EndCharIndex, endIndex);
		...
		} while (result.isEndOfParagraph() && result.nextParagraph()
	...
	private ZLTextLineInfo processTextLine(
		...
		if (cachedInfo != null) {
			applyControls(paragraphCursor, startIndex,
		...
		if ((currentElementIndex != endIndex)
				&& (ZLTextStyleCollection.Instance().getBaseStyle().AutoHyphenationOption
			...
			if (element instanceof ZLTextWord) {
				...
				if ((word.Length > 3)
						&& (spaceLeft > 2 * Context.getSpaceWidth())) {
					...
					for (; hyphenationPosition > 0; hyphenationPosition--) {
						if (hyphenationInfo
							...
							subwordWidth = getWordWidth(word, 0,
									hyphenationPosition, word.Data[word.Offset
											+ hyphenationPosition - 1] != '-');
	...
	private void prepareTextLine(ZLTextPage page, ZLTextLineInfo info, int y) {
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex; ++wordIndex, charIndex = 0) {
			...
			if (element == ZLTextElement.HSpace) {
				if (wordOccurred && (spaceCounter > 0)) {
					...
					if (getTextStyle().isUnderline()) {
						spaceElement = new ZLTextElementArea(paragraphIndex,
								wordIndex, 0, 0, false, false, getTextStyle(),
								element, x, x + spaceLength, y, y);
					} else {
						spaceElement = null;
					}
					x += spaceLength;
				...
				page.TextElementMap.add(new ZLTextElementArea(paragraphIndex,
						wordIndex, charIndex, length - charIndex, false,
		...
		if (!endOfParagraph) {
			...
			if (len > 0) {
				...
				page.TextElementMap.add(new ZLTextElementArea(paragraphIndex,
						wordIndex, 0, len, addHyphenationSign, changeStyle,
						getTextStyle(), word, x, x + width - 1, y - height + 1,
						y + descent));
			}
		}
	}
	...
	public final synchronized void gotoPosition(ZLTextPosition position) {
		if (position != null) {
			gotoPosition(position.getParagraphIndex(), position
					.getElementIndex(), position.getCharIndex());
		}
	}
	...
	private synchronized void preparePaintInfo(ZLTextPage page) {
		...
		case PaintStateEnum.TO_SCROLL_FORWARD:
			if (!page.EndCursor.getParagraphCursor().isLast()
					|| !page.EndCursor.isEndOfParagraph()) {
				...
				case ScrollingMode.SCROLL_PERCENTAGE:
					page.findPercentFromStart(startCursor, getTextAreaHeight(),
							myOverlappingValue);
					break;
				}
		...
		case PaintStateEnum.TO_SCROLL_BACKWARD:
			if (!page.StartCursor.getParagraphCursor().isFirst()
					|| !page.StartCursor.isStartOfParagraph()) {
				switch (myScrollingMode) {
				case ScrollingMode.NO_OVERLAPPING:
					page.StartCursor.setCursor(findStart(page.StartCursor,
							SizeUnit.PIXEL_UNIT, getTextAreaHeight()));
					break;
				case ScrollingMode.KEEP_LINES: {
					...
					if (!endCursor.isNull()) {
						ZLTextWordCursor startCursor = findStart(endCursor,
								SizeUnit.PIXEL_UNIT, getTextAreaHeight());
						if (startCursor.samePositionAs(page.StartCursor)) {
							page.StartCursor.setCursor(findStart(
				...
				case ScrollingMode.SCROLL_LINES:
					page.StartCursor.setCursor(findStart(page.StartCursor,
							SizeUnit.LINE_UNIT, myOverlappingValue));
					break;
				case ScrollingMode.SCROLL_PERCENTAGE:
					page.StartCursor.setCursor(findStart(page.StartCursor,
							SizeUnit.PIXEL_UNIT, getTextAreaHeight()
		...
		case PaintStateEnum.END_IS_KNOWN:
			page.StartCursor.setCursor(findStart(page.EndCursor,
					SizeUnit.PIXEL_UNIT, getTextAreaHeight()));
			buildInfos(page, page.StartCursor, page.EndCursor);
			break;
		}
	...
	private int paragraphSize(ZLTextWordCursor cursor,
		...
		while (wordIndex != endElementIndex) {
			ZLTextLineInfo info = processTextLine(paragraphCursor, wordIndex,
					charIndex, endElementIndex);
	...
	private void skip(ZLTextWordCursor cursor, int unit, int size) {
		...
		while (!cursor.isEndOfParagraph() && (size > 0)) {
			ZLTextLineInfo info = processTextLine(paragraphCursor, cursor
					.getElementIndex(), cursor.getCharIndex(), endElementIndex);
	...
	/*
	 * protected List<ZLTextElementArea> allElements() { return
	 * myCurrentPage.TextElementMap; }
	 */
	...
	protected boolean moveHyperlinkPointer(boolean forward) {
		...
		if (!hyperlinkAreas.isEmpty()) {
			final int index = hyperlinkAreas.indexOf(myCurrentHyperlink);
			if (index == -1) {
				myCurrentHyperlink = hyperlinkAreas.get(forward ? 0
						: hyperlinkAreas.size() - 1);
				return true;
			} else {
				if (forward) {
